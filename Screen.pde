...
class QueriesFlights extends Screen {
  ...
  QueriesFlights() {
    ...
    int cardW = width - 140;
    int cardH = 280;
    activeInput = ""; // either "departure" or "return"
...
void handleSuggestionsClick() {
  if (currentInput == null || suggestions.size() == 0) return;

  float x = currentInput.x;
  float y = currentInput.y + currentInput.h;
  float w = currentInput.w;
  float h = 35; // same as in drawSuggestions()

  for (int i = 0; i < suggestions.size(); i++) {
    float currentY = y + i * h;

    if (mouseX >= x && mouseX <= x + w &&
        mouseY >= currentY && mouseY <= currentY + h) {

      String selectedCity = suggestions.get(i).split(" - ")[1]; // <-- take only the city name
      currentInput.label = selectedCity;  // paste into the clicked input

      suggestions.clear(); // hide suggestions after selecting
  ...
  for (TextEntryButton b : allInputs) {
    if (b.over(mouseX, mouseY)) {
      ...
      // --- ADD THIS MINIMAL LINE ---
      if (b == inputFrom) activeInput = "departure";
      else if (b == inputTo) activeInput = "return";
    ...
    for (int i = 0; i < results.size(); i++) {
          ...
          mouseY > selectY && mouseY < selectY + selectH) 
      {
        ...
        if (!bookedFlights.contains(selected)) {
          ...
          flightConfirmedScreenObj.reset("Booking confirmed!");
